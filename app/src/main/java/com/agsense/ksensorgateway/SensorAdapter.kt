...
class SensorAdapter(private val onItemClick: (SensorReading) -> Unit = {}) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        // Door/light labels mix an emoji with a Hebrew word. Without an
        // explicit directional isolate, the RTL bidi algorithm can pair
        // an icon with the WRONG neighboring word when two such pairs
        // sit next to each other in one RTL-context line (e.g. showing
        // "🚪 אור  💡 סגורה" instead of "🚪 סגורה  💡 אור"). Wrapping each
        // pair in U+2066 (LRI) / U+2069 (PDI) keeps icon+label atomic.
        val LRI = "\u2066"
        val PDI = "\u2069"
