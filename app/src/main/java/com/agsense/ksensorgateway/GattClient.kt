 ...
 * humidity periodically" trigger and streams live readings.
 */
class GattClient(private val context: Context, private val listener: Listener) {
    ...
    // ---------------- Diagnostic: read device capabilities before guessing trigger params ----------------

    private val getParaBuffer = StringBuilder()

    private fun sendGetPara() {
        listener.onStatus("קורא פרמטרי מכשיר...")
        getParaBuffer.clear()
        val json = """{"msg":"getPara","type":1}"""
        sendJsonChunks(json.toByteArray(Charsets.US_ASCII)) { ack ->
            ...
            if (ack.size > 7) {
                getParaBuffer.append(String(ack.copyOfRange(7, ack.size), Charsets.US_ASCII))
            }
            if (pduTag == 2 || pduTag == 3) {
                finishGetPara()
            } else {
                pendingResponseHandler = { frame -> handleGetParaContinuation(frame) }
            }
        }
    }

    private fun handleGetParaContinuation(frame: ByteArray) {
        ...
        getParaBuffer.append(String(payload, Charsets.US_ASCII))
        ...
        if (isFinal) {
            ...
            pendingResponseHandler = { f -> handleGetParaContinuation(f) }
        }
    }

    private fun finishGetPara() {
        listener.onLog("getPara <- " + getParaBuffer.toString())
        sendCfgTrigger()
    }
    ...
    private fun sendCfgTrigger() {
        // Per the doc's own chapter order: section 3 "Enable sensor and log
        // recording" comes BEFORE section 4 "Enable humidity&temperature
        // trigger". We only sent the trigger message — without an active
        // sensor task, there may be nothing for the trigger to report.
        listener.onStatus("מפעיל את חיישן הלחות/טמפרטורה...")
        val enableSensorJson = """{"msg":"cfg","stype":1,"type":1,"sensor":2}"""
        sendJsonChunks(enableSensorJson.toByteArray(Charsets.US_ASCII)) { ack ->
            ...
            // The K6-supplement flat schema (stype:64, trType:8, htMsk, trAMode/
            // trAType/trAPrd) was tried and confirmed NOT to work on this
            // device: cfg was ack'd with cause=0x0 but no trigger data ever
            // arrived, even after waiting well past trATm. That device is
            // evidently speaking the newer KBPro "trObj" array schema
            // (section 7.4.1) instead, where trType is a single-purpose enum
            // and 0xC (12) means specifically "report temperature and
            // humidity to app periodically" — not 8, which under this schema
            // means "temperature above threshold" (a different trigger this
            // device may never fire). trAct 0x11 = 0x1 (start advertisement)
            // + 0x10 (report to connected app), matching the worked example
            // in section 7.4.2.1 of the spec.
            val triggerJson = """{"msg":"cfg","trObj":[{"trIdx":0,"trType":12,"trAct":17,"slot":0,"trATm":10}]}"""
            sendJsonChunks(triggerJson.toByteArray(Charsets.US_ASCII)) { ack2 ->
                val cause2 = if (ack2.size >= 7) ((ack2[5].toInt() and 0xFF) shl 8) or (ack2[6].toInt() and 0xFF) else -1
                val causeHex = "0x" + cause2.toString(16).uppercase()
                when (cause2) {
                    0x0, 0x5, 0x6 -> {
    ...
    // ---------------- Incoming trigger data (section 4.2 of the K6 supplement) ----------------
    ...
    private fun handleIndicateFrame(bytes: ByteArray) {
        ...
        // Section 4.2 of the K6 supplement (the trigger schema we're now
        // using) lists Humidity before Temperature in the notify frame.
