...
class GattClient(private val context: Context, private val listener: Listener) {
    ...
    private fun sendCfgTrigger() {
        ...
        sendJsonChunks(enableSensorJson.toByteArray(Charsets.US_ASCII)) { ack ->
            ...
            // getPara revealed trCap=528257 → bit11 set = "temperature
            // realtime trigger" IS supported (trType 12 was the right
            // choice). But the previous attempt combined it with
            // trAct:17 (0x11 = start-advertisement + report-to-app), which
            // requires "slot"+"trATm" per the spec since trAct&0x1 is set —
            // and slot 0 was never configured with any advertisement
            // (advObj), which likely caused the illegal-content rejection.
            // We only need the connected app to receive the reading, not to
            // also kick off a beacon advertisement burst, so drop bit0 and
            // use trAct:16 (0x10, report-to-app only) — that condition is
            // no longer triggered, so slot/trATm aren't needed at all.
            val triggerJson = """{"msg":"cfg","trObj":[{"trIdx":0,"trType":12,"trAct":16}]}"""
