...
class GattClient(private val context: Context, private val listener: Listener) {
    ...
    private fun sendCfgTrigger() {
        ...
        sendJsonChunks(enableSensorJson.toByteArray(Charsets.US_ASCII)) { cause ->
            ...
            // Using the K6 "supplement" flat schema (section 4.1) rather than
            // the KBPro trObj-array schema: trType 8 here means "the humidity
            // trigger" (not "temperature above" as in the KBPro trObj table),
            // and htMsk 0x10 = KBTriggerHTParaMaskRealtimeRpt2App, i.e. report
            // every measurement interval. This schema does NOT take a "slot"
            // field — trAMode/trAType/trAPrd belong here instead. Mixing in a
            // "slot" field (as the previous version did) doesn't match either
            // documented schema, which is likely why cfg was ack'd (cause=0x0,
            // the parser just ignores unknown fields) but no data ever arrived.
            val triggerJson = """{"msg":"cfg","stype":64,"trType":8,"trAct":17,"htMsk":16,"trAMode":0,"trAType":1,"trATm":10,"trAPrd":1000}"""
