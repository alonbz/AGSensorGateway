...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val debugSection: View = findViewById(R.id.debugSection)
        val toggleDebugButton: Button = findViewById(R.id.buttonToggleDebug)
        toggleDebugButton.setOnClickListener {
            val nowVisible = debugSection.visibility != View.VISIBLE
            debugSection.visibility = if (nowVisible) View.VISIBLE else View.GONE
            toggleDebugButton.text = if (nowVisible) "הסתר פרטים טכניים" else "הצג פרטים טכניים (GATT / יומן)"
        }
        ...
        buttonGattConnect.setOnClickListener {
            ...
            val hexOnly = rawMac.replace(Regex("[:\\s-]"), "")
            if (!hexOnly.matches(Regex("^[0-9A-F]{12}$"))) {
                Toast.makeText(this, "פורמט MAC לא תקין, לדוגמה BC:57:29:26:BD:EF (עם או בלי נקודותיים)", Toast.LENGTH_LONG).show()
        ...
        buttonExploreGatt.setOnClickListener {
            ...
            val hexOnly = rawMac.replace(Regex("[:\\s-]"), "")
            if (!hexOnly.matches(Regex("^[0-9A-F]{12}$"))) {
                Toast.makeText(this, "פורמט MAC לא תקין, לדוגמה C4:A8:28:65:08:75 (עם או בלי נקודותיים)", Toast.LENGTH_LONG).show()
                return@setOnClickListener
            }
            val mac = hexOnly.chunked(2).joinToString(":")
            if (!hasAllPermissions()) {
