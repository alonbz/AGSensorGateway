...
class MainActivity : AppCompatActivity() {
    ...
    private fun handleScanResult(result: ScanResult) {
        ...
        // Unconditional, once-per-address heartbeat: confirms the scan
        // callback is receiving *something*, independent of whether it
        // passes the "looksRelevant" filter below. If this never
        // appears, the scan itself isn't seeing any advertisements
        // (Bluetooth off, out of range, or OS-level scan throttling) —
        // if it appears but the known MAC is never among them, the
        // sensor's advertisements aren't reaching this scan callback
        // at all (possibly a connectable-only/non-scannable mode).
        ...
        // NOTE: on Android 12+, reading the device name requires the
        // separate BLUETOOTH_CONNECT permission (not just SCAN) — if
        // that isn't granted, name comes back null and a name-based
        // filter alone can silently match nothing even with the
        // sensor in range. So we also match on the known MAC address
        // as a permission-independent fallback.
        val knownMac = "BC:57:29:26:BD:EF"
        val looksRelevant = (deviceName?.contains("KB", ignoreCase = true) == true) ||
            (deviceName?.contains("KSensor", ignoreCase = true) == true) ||
            result.device.address.equals(knownMac, ignoreCase = true) ||
