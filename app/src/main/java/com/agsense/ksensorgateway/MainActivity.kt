...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        buttonExploreGatt.setOnClickListener {
            ...
            val mac = hexOnly.chunked(2).joinToString(":")
            appendLog("[explorer] MAC מנורמל: $mac. בודק הרשאות...")
            val missing = requiredPermissions().filter {
                ContextCompat.checkSelfPermission(this, it) != PackageManager.PERMISSION_GRANTED
            }
            if (missing.isNotEmpty()) {
                appendLog("[explorer] !!! הרשאות חסרות: ${missing.joinToString(", ") { it.substringAfterLast('.') }} - מבקש הרשאות ועוצר כאן")
                ActivityCompat.requestPermissions(this, requiredPermissions(), REQUEST_PERMISSIONS_CODE)
                return@setOnClickListener
            }
            appendLog("[explorer] הרשאות תקינות. מתחיל חיבור...")
            try {
