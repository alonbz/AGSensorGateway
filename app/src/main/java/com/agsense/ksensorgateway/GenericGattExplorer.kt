 ...
 *
 * IMPORTANT — operations run ONE AT A TIME via a queue, never fired in
 * a loop: Android's BluetoothGatt only supports a single in-flight GATT
 * operation (read / descriptor-write / characteristic-write) per
 * connection at once. Issuing a second one before the first one's
 * callback has returned makes the second call return false and do
 * nothing — silently, no error logged by the platform. The original
 * version of this class looped over every discovered characteristic and
 * fired setCharacteristicNotification+writeDescriptor and
 * readCharacteristic back-to-back for all of them in one pass; in
 * practice ONLY the very first operation in that loop ever completed
 * (confirmed live: the log always stopped right after "read
 * <the first characteristic>", no matter how many services/characteristics
 * the device exposed). Every discovered operation below is queued and
 * the next one only starts once the previous one's callback fires.
 */
class GenericGattExplorer(private val context: Context, private val listener: Listener) {
    ...
    private sealed class PendingOp {
        data class Subscribe(val characteristic: BluetoothGattCharacteristic) : PendingOp()
        data class Read(val characteristic: BluetoothGattCharacteristic) : PendingOp()
    }

    private var gatt: BluetoothGatt? = null
    private val opQueue = mutableListOf<PendingOp>()
     ...
     *
     * This is a manual one-off action triggered by the person, separate
     * from the discovery queue below — if it's fired while that queue is
     * still draining, the two could in principle collide the same way
     * the old code did. In practice the queue drains in well under a
     * second, so this hasn't been an issue.
     */
    @SuppressLint("MissingPermission")
    fun writeCharacteristic(uuidFragment: String, hexBytes: String) {
        ...
        for (service in g.services) {
            for (ch in service.characteristics) {
                if (ch.uuid.toString().contains(uuidFragment.lowercase())) {
                    target = ch
                    break
                }
            }
    ...
    /** Pops and starts the next queued operation; no-op (with a final log) once the queue is empty. */
    @SuppressLint("MissingPermission")
    private fun processNextOp() {
        val g = gatt ?: return
        if (opQueue.isEmpty()) {
            listener.onLog("[explorer] === התור הסתיים (כל ה-read/subscribe רצו). עכשיו תפעיל דלת/אור אם רלוונטי. ===")
            return
        }
        val op = opQueue.removeAt(0)
        when (op) {
            is PendingOp.Subscribe -> {
                val ch = op.characteristic
                ...
                if (cccd != null) {
                    ...
                    listener.onLog("[explorer]   ${ch.uuid} -> נרשם ל-notify/indicate (setNotification=$notifyOk, writeDescriptor started=$started)")
                    if (!started) processNextOp() // nothing async will call us back; advance ourselves
                } else {
                    listener.onLog("[explorer]   ${ch.uuid} -> יש NOTIFY אבל אין CCCD descriptor (setNotification=$notifyOk)")
                    processNextOp() // nothing to wait for
                }
            }
            is PendingOp.Read -> {
                val ch = op.characteristic
                ...
                if (!started) {
                    listener.onLog("[explorer]   ${ch.uuid} -> readCharacteristic נכשל להתחיל")
                    processNextOp()
                }
            }
        }
    ...
    private val callback = object : BluetoothGattCallback() {
        ...
        override fun onServicesDiscovered(g: BluetoothGatt, status: Int) {
            ...
            for (service in g.services) {
                ...
                for (ch in service.characteristics) {
                    ...
                    if (canNotify) opQueue.add(PendingOp.Subscribe(ch))
                    ...
                    if ((ch.properties and BluetoothGattCharacteristic.PROPERTY_READ) != 0) {
                        opQueue.add(PendingOp.Read(ch))
                    }
                }
            }
            listener.onLog("[explorer] === תור בגודל ${opQueue.size} (read+subscribe) — מריץ אחד-אחד ===")
            processNextOp()
        }
        ...
        override fun onCharacteristicRead(g: BluetoothGatt, ch: BluetoothGattCharacteristic, status: Int) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                listener.onLog("[explorer] read ${ch.uuid} נכשל, status=$status")
            }
            processNextOp()
        }
        ...
        override fun onDescriptorWrite(g: BluetoothGatt, descriptor: BluetoothGattDescriptor, status: Int) {
            ...
            listener.onLog("[explorer]   subscribe-write ${descriptor.characteristic?.uuid}: $statusText")
            processNextOp()
        }
